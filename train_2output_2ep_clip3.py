...
from utils import get_ids, split_ids, split_train_val, get_imgs_and_masks_2ep_clip3, batch, split_train_val_test
from BCELoss_weight import BCELoss_weight
import re
def extract_num(string):
    string_list = re.findall(r'-?\d+\.?\d*e?-?\d*?', string)
...
def train_net(net,
    ...
    classweight = extract_num(classweightstr)
    # classweight = [1, 5, 5]
    ...
    for epoch in range(epochs):
        ...
        train = get_imgs_and_masks_2ep_clip3(iddataset['train'], dir_img, dir_mask, img_scale)
        val = get_imgs_and_masks_2ep_clip3(iddataset['val'], dir_img, dir_mask, img_scale)
        ...
        for i, b in enumerate(batch(train, batch_size)):
            ...
            true_masks = np.transpose(true_masks, axes=[0,3,1,2])  # 10 5 240 240
...
def get_args(raw_args=None):
    ...
    parser.add_option("-w", "--bceloss-weight", default= '[1, 3, 3]',
                      action="store", type="string", dest="bcelossweightstr")
...
def train_5output(raw_args=None):
    ...
    # args.batchsize=20
    # args.scale=0.5
    classweightstr = args.bcelossweightstr
...
if __name__ == '__main__':
    ...
    # lrd = 0.87
    #
    # inputcode = ['-i', '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/img', '-m',
    #              '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/mask', '-v',
    #              '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/cp06/', '-e', str(epoch), '-l', str(lr),
    #              '-d', str(lrd)]
...
##
    ...
    #              '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/cp04/', '-e', str(epoch), '-l', str(lr), '-d', str(lrd)]
    # train_5output(inputcode)
...
##
    ...
    #
    # inputcode = ['-i', '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/img', '-m', '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/mask', '-v',
    #              '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/cp05/', '-e', str(epoch), '-l', str(lr), '-d', str(lrd)]
    # train_5output(inputcode)
    ...
    lrd = 0.9
    classweight = '[1, 5, 5]'
    savedirname = 'lr' + str(lr) + '.lrd' + str(lrd) + '.weight' + classweight + '/'
    savepath = '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/' + savedirname
    safecreate(savepath)

    inputcode = ['-i', '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/img', '-m',
                 '/home/zhaojin/data/TacomaBridge/segdata/girder2cls_data_2ep/mask', '-v',
                 savepath, '-e', str(epoch), '-l', str(lr),
                 '-d', str(lrd)]
    train_5output(inputcode)
